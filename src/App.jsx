...
function GameInSlot({ user, slotIndex, isNew, difficulty, onExit }) {
  const { state, dispatch } = useGame()
  const [ready, setReady] = useState(false)

  useEffect(() => {
    if (isNew) {
      dispatch(startNewGame(difficulty || 'medium'))
    } else {
      const slot = getSlot(user.id, slotIndex)
      if (slot?.state) {
        dispatch({ type: 'LOAD_GAME', payload: { savedState: slot.state } })
      } else {
        dispatch(startNewGame(difficulty || 'medium'))
      }
    }
